...
Manipulate the YAML front matter in Obsidian notes with Emacs.
...
You can customize settings in the =package-name= group.
  ...
  + =oyt-time-string-format= : Format for how time strings are displayed.

  + =oyt-storage-type= : Data structure for internal storage of YAML data.
